...
export default function AdminDashboard() {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Sidebar */}
      <aside className="w-64 border-r border-white/5 bg-[#0d1321] flex flex-col">
        <div className="p-5 border-b border-white/5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center text-lg">🌐</div>
            <div>
        ...
        <nav className="flex-1 p-3 space-y-1">
          ...
          <NavItem icon="👥" label="Personal" />
          <NavItem icon="🌍" label="Eventos Sísmicos" />
          <NavItem icon="📋" label="Reportes SAR" />
          <NavItem icon="📡" label="Fuentes Sísmicas" />
          <NavItem icon="⚙️" label="Configuración" />
        </nav>
        <div className="p-4 border-t border-white/5">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-purple-600 flex items-center justify-center text-xs font-bold">AD</div>
            <div>
...
function NavItem({ icon, label, active }: { icon: string; label: string; active?: boolean }) {
  return (
    <div className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm cursor-pointer transition ${
