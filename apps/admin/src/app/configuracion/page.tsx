    ...
    <>
      ...
      <div className="p-6 space-y-6 max-w-4xl">
        ...
        <Section title="⚡ Sistema de Alertas" description="Configuración del disparo automático de alertas">
          <ToggleRow
            label="Alerta automática por sismo"
            description="Disparar alerta al personal cuando se detecta un sismo significativo"
            value={config.alertaAutomatica}
            onChange={(v) => handleChange('alertaAutomatica', v)}
          />
          <InputRow
            label="Umbral de magnitud"
            description="Magnitud mínima para disparar alerta automática"
            value={config.umbralMagnitud}
            onChange={(v) => handleChange('umbralMagnitud', v)}
            suffix="M"
          />
          <InputRow
            label="Radio de zona de impacto"
            description="Distancia máxima del epicentro para considerar personal en riesgo"
            value={config.zonaRadio}
            onChange={(v) => handleChange('zonaRadio', v)}
            suffix="km"
          />
        </Section>
        ...
        <Section title="📍 Monitoreo de Personal" description="Parámetros de seguimiento y check-in">
          <InputRow
            ...
            description="Tiempo entre verificaciones automáticas de estado del personal"
            value={config.intervaloCheckin}
            onChange={(v) => handleChange('intervaloCheckin', v)}
            suffix="min"
          />
          <InputRow
            label="Tiempo sin respuesta"
            description="Minutos sin respuesta antes de marcar como 'Sin respuesta'"
            value={config.tiempoSinRespuesta}
            onChange={(v) => handleChange('tiempoSinRespuesta', v)}
            suffix="min"
          />
          <InputRow
            label="Timeout alerta SOS"
            description="Minutos para confirmar recepción de SOS antes de escalar"
            value={config.tiempoAlertaSOS}
            onChange={(v) => handleChange('tiempoAlertaSOS', v)}
            suffix="min"
          />
        </Section>
        ...
        <Section title="🔔 Notificaciones" description="Canales de comunicación con el personal">
          <ToggleRow
            ...
            description="Enviar alertas push a la app móvil del personal"
            value={config.notificacionesPush}
            onChange={(v) => handleChange('notificacionesPush', v)}
          />
          <ToggleRow
            label="Notificaciones Email"
            description="Enviar resúmenes y alertas por correo electrónico"
            value={config.notificacionesEmail}
            onChange={(v) => handleChange('notificacionesEmail', v)}
          />
          <ToggleRow
            label="Notificaciones SMS"
            description="Enviar alertas críticas por SMS (requiere créditos Twilio)"
            value={config.notificacionesSMS}
            onChange={(v) => handleChange('notificacionesSMS', v)}
          />
        </Section>
        ...
        <Section title="🔗 Integraciones" description="Tokens y claves de servicios externos">
          <InputRow
            label="Mapbox Token"
            description="Token de acceso para el mapa vectorial en vivo"
            value={config.mapboxToken}
            onChange={(v) => handleChange('mapboxToken', v)}
        ...
        <div className="glass rounded-2xl p-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <InfoItem label="Base de datos" value="No conectada" />
            <InfoItem label="API Backend" value="No conectada" />
          </div>
        </div>
      </div>
    </>
